...
try:
    ...
    try:
        from services.cdss_dokter import analyze_icd10_dan_tatalaksana as _cdss_dokter_analyze
        CDSS_DOKTER_AVAILABLE = True
    except ImportError:
        CDSS_DOKTER_AVAILABLE = False
        _cdss_dokter_analyze = None
import pandas as pd
import plotly.express as px
import requests
import streamlit as st

# Import dari modul autentikasi baru Anda
from modules.auth_provider import get_auth_provider
from modules.bridge_updated import display_user_badge
from modules.bridge_cpoe_sync import CPOESyncManager

# ── 1. KONFIGURASI HALAMAN (Wajib di paling atas & hanya 1 kali) ──────────────
st.set_page_config(page_title="SMART_EMR - SMART_CARE ", page_icon="🫀", layout="wide")


# ── 2. KONSTANTA SISTEM & DATABASE KLINIS ────────────────────────────────────
DB_PATH     = "rsjpdhk_emr.db"  # Database klinis CPPT (terpisah dari users.db auth)
API_URL     = "http://127.0.0.1:8000/api/v1/extract"
LAB_API_URL = "http://127.0.0.1:8000/api/v1/analyze-cppt"
API_TIMEOUT = 3   # detik
SESSION_TTL = 30  # menit — durasi sesi sebelum auto-logout




# ── SISTEM LOGIN TERPUSAT ─────────────────────────────────────────────────────
# Satu-satunya titik masuk autentikasi. Menggunakan auth_provider + auth_system.
# Tidak ada KREDENSIAL hardcoded, tidak ada dapatkan_semua_personel().

_auth_provider = get_auth_provider()

if not st.session_state.get("authenticated", False):
    from modules.auth_system import render_login_form
    _, col_center, _ = st.columns([1, 1.8, 1])
    with col_center:
        st.write("<br><br>", unsafe_allow_html=True)
        with st.container(border=True):
            st.markdown(
                "<h2 style='text-align:center;margin-bottom:0'>🫀 Smart EMR</h2>",
                unsafe_allow_html=True,
            )
            st.markdown(
                "<h4 style='text-align:center;font-weight:normal;margin-top:5px;color:#555'>"
                "Smart Care Plan — RSJPDHK</h4>",
                unsafe_allow_html=True,
            )
            st.markdown("<hr style='margin:10px 0 20px'>", unsafe_allow_html=True)
            render_login_form(_auth_provider)
        st.markdown(
...
# ── Pastikan logged_in tersinkron (kompatibilitas sub-modul lama) ─────────────
if not st.session_state.get("logged_in"):
    st.session_state["logged_in"] = True

# ── Badge user di sidebar ─────────────────────────────────────────────────────
display_user_badge()



# ── 6. SPEECH RECOGNITION INITIALIZATION ─────────────────────────────────────
try:
...
# ── Initialize CPOE Sync Manager ─────────────────────────────────────
try:
    cpoe_sync = CPOESyncManager(db_path=DB_PATH)
except Exception as e:
    logger.warning(f"CPOESyncManager init error: {e}")
    cpoe_sync = None


# ── 8. OLLAMA / QWEN AI CONFIGURATION (On-Premise) ───────────────────────────
# Gunakan Ollama yang berjalan lokal (on-premise).
# Pastikan Ollama sudah running: `ollama serve`
# dan model sudah ditarik:      `ollama pull qwen2.5`
#
# Untuk mengubah URL atau model, set environment variable:
#   OLLAMA_BASE_URL  → default: http://localhost:11434
#   OLLAMA_MODEL     → default: qwen2.5:1.5b

def _get_ollama_config() -> tuple[str, str]:
    """Kembalikan (base_url, model_name) dari secrets/env, dengan default lokal."""
    try:
        base_url = st.secrets.get("OLLAMA_BASE_URL", "http://localhost:11434")
        model    = st.secrets.get("OLLAMA_MODEL",    "qwen2.5:1.5b")
    except Exception:
...
_OLLAMA_BASE_URL, _OLLAMA_MODEL = _get_ollama_config()
_ollama_client = None  # placeholder agar referensi lama tidak error


def _ollama_generate(system_prompt: str, user_text: str,
                     num_predict: int = 512, temperature: float = 0.1,
                     timeout: int = 45) -> str:
    """
    Wrapper Ollama via /api/generate (kompatibel semua versi Ollama).
    Raise RuntimeError dengan pesan yang jelas jika model tidak merespons.
    """
    ...
    try:
        ...
        raise RuntimeError(
            f"Tidak dapat terhubung ke Ollama di {_OLLAMA_BASE_URL}. "
            "Pastikan Ollama sudah berjalan: jalankan 'ollama serve' di terminal."
        )
    except requests.exceptions.HTTPError as e:
        raise RuntimeError(
            f"Ollama HTTP error {resp.status_code}: {e}. "
            "Coba jalankan: ollama serve"
        )

    data = resp.json()
    # Tangani dua format respons: streaming-done object vs langsung
    result = data.get("response", "").strip()
    if not result:
        # Cek apakah model tidak ditemukan
        err = data.get("error", "")
        if err:
            raise RuntimeError(
                f"Ollama error: {err}\n"
                f"Model '{_OLLAMA_MODEL}' mungkin belum di-pull. "
                f"Jalankan: ollama pull {_OLLAMA_MODEL}"
            )
        raise RuntimeError(
            f"Ollama mengembalikan respons kosong untuk model '{_OLLAMA_MODEL}'. "
            f"Pastikan model sudah tersedia dengan: ollama list"
        )
    return result

# Cek konektivitas Ollama saat startup (non-blocking)
try:
    _health = requests.get(f"{_OLLAMA_BASE_URL}/api/tags", timeout=2)
    if _health.status_code == 200:
        AI_NORMALIZE_AVAILABLE = True
        logger.info("Ollama tersedia di %s — model: %s", _OLLAMA_BASE_URL, _OLLAMA_MODEL)
    else:
        raise RuntimeError(f"Ollama health check HTTP {_health.status_code}")
...
# =============================================================================
# LOGIKA STRUKTUR GRAFIK KLINIS, SELEKSI PASIEN & FORMAT CPPT KODE LAMA ANDA...
# =============================================================================
st.info("Sistem Otentikasi Sidik Jari Terintegrasi Berhasil Dimuat.")
# ====================================================================
...
def normalize_clinical_transcript(raw_text: str, field: str = "S") -> str:
    ...
    system_prompt = (
        "Kamu adalah asisten normalisasi catatan keperawatan kardiovaskular. "
        "Tugasmu: bersihkan hasil speech-to-text berikut menjadi catatan medis yang rapi.\n\n"
        "ATURAN KETAT:\n"
        "1. Output HANYA teks yang sudah dibersihkan. Dilarang menulis penjelasan atau komentar apapun.\n"
        "2. Hapus kata filler dan pengulangan: 'eee', 'hmm', 'anu', 'yang', 'tadi', 'jadi', dll.\n"
        "3. Ubah angka ucapan menjadi digit:\n"
        "   - 'sembilan puluh delapan persen' → '98%'\n"
        "   - 'seratus empat puluh per sembilan puluh' → '140/90'\n"
        "   - 'delapan puluh kali per menit' → '80x/menit'\n"
        "4. Standarisasi istilah klinis:\n"
        "   - 'es pe o dua' / 'spo dua' → 'SpO2'\n"
        "   - 'efe ejeksi' / 'ejeksi fraksi' → 'EF'\n"
        "   - 'sesak nafas' → 'sesak napas'\n"
        "   - 'tekanan darah' disingkat 'TD', 'nadi' disingkat 'N', 'suhu' disingkat 'S'\n"
        "   - 'per menit' → 'x/menit'\n"
        "5. Dilarang menambahkan diagnosa, interpretasi, atau informasi baru yang tidak diucapkan.\n"
        "6. Istilah medis baku (SpO2, EF, LVEF, EKG, ICU, dll.) biarkan apa adanya.\n\n"
        "CONTOH BENAR:\n"
        "Input: 'pasien eee mengeluh sakit dada sakit dada sejak tadi tiga puluh menit yang lalu'\n"
        "Output: Pasien mengeluh sakit dada sejak 30 menit yang lalu.\n\n"
        "Input: 'tekanan darah seratus dua puluh per delapan puluh nadi delapan puluh kali per menit'\n"
        "Output: TD 120/80 mmHg, N 80x/menit.\n\n"
        f"Konteks: data {field_label} pada pasien jantung. Sekarang bersihkan teks berikut:"
    )
    ...
    try:
        ...
        logger.warning("normalize_clinical_transcript (Ollama) error: %s — memakai teks mentah", exc)
        return raw_text


def _detect_language_heuristic(text: str) -> str:
    """
    Deteksi bahasa sederhana berbasis heuristik kata umum.
    Mengembalikan 'en' jika terindikasi Inggris, 'id' untuk lainnya.
    Digunakan sebelum memanggil Ollama agar tidak wasteful.
    """
    if not text or len(text.strip()) < 5:
        return "id"
    en_markers = {
        "the", "patient", "complains", "reports", "denies", "presents",
        "with", "without", "history", "of", "no", "mild", "severe",
        "shortness", "breath", "chest", "pain", "palpitations", "swelling",
        "blood", "pressure", "heart", "rate", "oxygen", "saturation",
        "rhythmic", "irregular", "normal", "abnormal", "bilateral",
        "and", "or", "is", "are", "was", "were", "has", "have",
    }
    words = set(re.findall(r"[a-zA-Z]+", text.lower()))
    en_hits = len(words & en_markers)
    # Jika >=3 kata Inggris umum ditemukan → kemungkinan besar bahasa Inggris
    return "en" if en_hits >= 3 else "id"
...
# =============================================================================
# SESSION STATE — KLINIS (non-auth)
# =============================================================================
# Key AUTH dikelola eksklusif oleh auth_system.set_auth_session() /
# clear_auth_session(). Jangan tambahkan key auth di sini.

def init_session() -> None:
    """Inisialisasi session state klinis (pasien, CPPT, CDSS, VTT).
    ...
    defaults = {
        ...
        # ── Audit darurat (log bypass hardware scanner) ───────────────────
        "emergency_logs":   [],
    }
...
def logout(reason: str = "") -> None:
    """
    Logout terpusat.
    1. Hapus semua auth session via clear_auth_session() (auth_system).
    2. Reset session klinis (pasien, CPPT, CDSS).
    """
    from modules.auth_system import clear_auth_session
    clear_auth_session()
    ...
    clinical_keys = [
        ...
        "draft_cppt_apoteker", "draft_cppt_gizi",  # FIX RBAC: panel baru Apoteker/Gizi
    ]
...
def set_active_patient(episode_id: str, pasien_data: dict | None = None) -> None:
    ...
    st.session_state.update({
        ...
        "last_audio_s_id":  None,
        "last_audio_o_id":  None,
        # FIX RBAC: draft catatan Apoteker/Gizi juga harus direset per-pasien
...
def generate_catatan_apoteker(s_input: str, o_input: str) -> str:
    """
    Susun draf Catatan Pelayanan Kefarmasian (CPPT Farmasi).
    CATATAN: ini BUKAN mesin CDSS — murni penyusun narasi dari input manual
    Apoteker (tinjauan resep, interaksi obat, kesesuaian dosis). Algoritme
    CDSS hanya berlaku untuk Perawat (formulasikan_asuhan) dan Dokter
    (analyze_clinical_trends_improved) sesuai arahan, dan keduanya tidak
    diubah oleh fungsi ini.
    """
    nama_apoteker = str(st.session_state.get("nama_lengkap") or st.session_state.get("user_id", "Apoteker")).upper()
    episode_id    = st.session_state.get("episode_id", "-")
    shift         = st.session_state.get("shift", "-")
    ...
    draft  = "CATATAN PELAYANAN KEFARMASIAN (CPPT FARMASI)\n"
    draft += f"ID Episode : {episode_id} | Shift: {shift}\n"
    draft += f"Tanggal    : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    draft += f"Nama Pasien: {pasien_nama} | No. RM: {pasien_rm} | Ruangan: {pasien_ruang}\n"
    draft += "=" * 70 + "\n\n"
    draft += f"S: {s_input.strip()}\n"
    draft += f"O: {o_input.strip()}\n\n"
    draft += (
        "A (Tinjauan Farmasi): Lengkapi penilaian interaksi obat, duplikasi "
        "terapi, dan kesesuaian dosis berdasarkan order CPOE di atas.\n"
    )
    draft += "P (Rencana Farmasi): Lengkapi rekomendasi penyesuaian terapi (jika ada).\n\n"
    draft += f"Divalidasi oleh: {nama_apoteker} (Apoteker)\n"
    return draft


def generate_catatan_gizi(s_input: str, o_input: str) -> str:
    """
    Susun draf Catatan Asuhan Gizi (CPPT Gizi).
    CATATAN: ini BUKAN mesin CDSS — murni penyusun narasi dari input manual
    Ahli Gizi (asesmen status gizi & rencana diet). Lihat catatan di
    generate_catatan_apoteker() di atas mengenai cakupan algoritme CDSS.
    """
    nama_gizi    = str(st.session_state.get("nama_lengkap") or st.session_state.get("user_id", "Ahli Gizi")).upper()
    episode_id   = st.session_state.get("episode_id", "-")
    shift        = st.session_state.get("shift", "-")
    pasien_nama  = st.session_state.get("pasien_nama", "-")
    pasien_rm    = st.session_state.get("pasien_no_rm", "-")
    ...
    draft  = "CATATAN ASUHAN GIZI (CPPT GIZI)\n"
    draft += f"ID Episode : {episode_id} | Shift: {shift}\n"
    draft += f"Tanggal    : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    draft += f"Nama Pasien: {pasien_nama} | No. RM: {pasien_rm} | Ruangan: {pasien_ruang}\n"
    draft += "=" * 70 + "\n\n"
    ...
    draft += f"O: {o_input.strip()}\n\n"
    draft += (
        "A (Asesmen Gizi): Lengkapi status gizi, kebutuhan kalori/protein, "
        "dan diagnosis gizi pasien.\n"
    )
    draft += "P (Rencana Diet): Lengkapi rencana intervensi & jenis diet yang direkomendasikan.\n\n"
    draft += f"Divalidasi oleh: {nama_gizi} (Ahli Gizi)\n"
    return draft
...
def main_app() -> None:
    ...
    # ── Role-based sidebar: Perawat input SLKI, Dokter lihat diagnosa ICD-10 aktif ──
    # FIX RBAC: sebelumnya biner (Dokter vs else) -> sekarang 4 profesi inti CPPT,
    # masing-masing dengan flag eksplisit. Algoritme CDSS Perawat & Dokter TIDAK disentuh
    # sama sekali -- hanya ditambah jalur baru utk Apoteker & Ahli Gizi.
    _current_role = st.session_state.get("role", "Perawat")
    IS_DOKTER   = _current_role == "Dokter"
    IS_PERAWAT  = _current_role == "Perawat"
    IS_APOTEKER = _current_role == "Apoteker"
    IS_GIZI     = _current_role == "Ahli Gizi"
    # Role di luar 4 profesi inti (Admin/Radiolog/Laborat) -- biasanya hanya masuk
    # lewat Mode Darurat -- diberi tampilan baca-saja, tidak dianggap Perawat.
    IS_LAINNYA  = not (IS_DOKTER or IS_PERAWAT or IS_APOTEKER or IS_GIZI)

    if IS_DOKTER:
        # Panel Dokter: daftar diagnosis ICD-10 aktif dari session_state
        with st.sidebar.expander("🔬 Diagnosis ICD-10 Aktif", expanded=True):
            diagnosa_aktif = st.session_state.get("diagnosa_aktif", [])
            if diagnosa_aktif:
                for dx in diagnosa_aktif:
                    kode_icd = dx.get("kode", dx.get("kode_icd10", ""))
                    nama_dx  = dx.get("nama", dx.get("nama_diagnosa", ""))
                    st.markdown(f"🔷 `{kode_icd}` — {nama_dx}")
            else:
                st.caption("Belum ada diagnosis ICD-10 pada episode ini.")
                st.caption("Gunakan tombol **Analisis ICD-10** di bawah setelah mengisi data S/O.")
    elif IS_PERAWAT:
        # Panel Perawat: input skor SLKI
        ...
        # Panel Apoteker: ringkasan order obat aktif (untuk tinjauan farmasi)
        with st.sidebar.expander("💊 Order Obat Aktif", expanded=True):
            jumlah_obat = 0
            if cpoe_sync:
                try:
                    orders = cpoe_sync.get_cpoe_orders(episode_id=st.session_state.episode_id) or []
                    jumlah_obat = len([o for o in orders if o.get("order_type") == "obat"])
                except Exception:
                    jumlah_obat = 0
            if jumlah_obat:
                st.metric("Order Obat Menunggu Tinjauan", jumlah_obat)
            else:
                st.caption("Belum ada order obat dari dokter untuk episode ini.")
            st.caption("Detail order tersedia di panel **Order dari Dokter (CPOE)** di bawah.")
    elif IS_GIZI:
        # Panel Ahli Gizi: pengingat status nutrisi
        with st.sidebar.expander("🥗 Ringkasan Status Gizi", expanded=True):
            st.caption(
                "Isi **Asesmen Gizi** & **Rencana Diet** di form utama untuk "
                "mendokumentasikan asuhan gizi episode ini."
            )
    else:
        st.sidebar.caption("ℹ️ Tidak ada panel khusus untuk role ini.")
    ...
    # ── CPOE Orders Display Section (Perawat & Apoteker meninjau order Dokter) ────
    if cpoe_sync and (IS_PERAWAT or IS_APOTEKER) and "episode_id" in st.session_state:
        with st.container(border=True):
            st.subheader("📋 Order dari Dokter (CPOE) — Terintegrasi")
            
            try:
                orders = cpoe_sync.get_cpoe_orders(
                    episode_id=st.session_state.episode_id
                )
                
                if orders and len(orders) > 0:
                    st.success(f"✅ {len(orders)} order ditemukan")
                    
                    # Group by type for display
                    obat_orders = [o for o in orders if o.get('order_type')=='obat']
                    lab_orders = [o for o in orders if o.get('order_type')=='lab']
                    vent_orders = [o for o in orders if o.get('order_type')=='ventilator']
                    bundle_orders = [o for o in orders if o.get('order_type')=='bundle']
                    
                    tabs = st.tabs([
                        f"💊 Obat ({len(obat_orders)})",
                        f"🧪 Lab ({len(lab_orders)})",
                        f"🫁 Ventilator ({len(vent_orders)})",
                        f"📦 Bundle ({len(bundle_orders)})"
                    ])
                    
                    order_data = [
                        ("obat", obat_orders),
                        ("lab", lab_orders),
                        ("ventilator", vent_orders),
                        ("bundle", bundle_orders)
                    ]
                    
                    for tab, (order_type, filtered_orders) in zip(tabs, order_data):
                        with tab:
                            if filtered_orders:
                                st.dataframe(
                                ...
                                with col1:
                                    if st.button(f"✅ Validasi & Integrasikan", key=f"validate_{order_type}"):
                                        try:
                                            cpoe_sync.update_order_status(
                                                episode_id=st.session_state.episode_id,
                                                order_type=order_type,
                                                status="integrated"
                                            )
                                            st.success("✅ Order integrated")
                                            st.rerun()
                                        except Exception as e:
                                            st.error(f"Error: {e}")
                                with col2:
                                    if st.button(f"❌ Tolak", key=f"reject_{order_type}"):
                                        try:
                                            cpoe_sync.update_order_status(
                                                episode_id=st.session_state.episode_id,
                                                order_type=order_type,
                                ...
                                st.info(f"No {order_type} orders")
                else:
                    st.info("📭 Belum ada order dari dokter untuk episode ini")
                    
            except Exception as e:
                st.warning(f"⚠️ Error loading CPOE orders: {e}")
        
        st.write("")  # spacer

    # ── Role-based konten utama: Perawat → grafik SLKI, Dokter → panel ICD-10 CDSS,
    #    Apoteker → tinjauan farmasi, Ahli Gizi → asuhan gizi (FIX RBAC 4 profesi) ──
    if IS_DOKTER:
        # ── CDSS DOKTER: Panel ICD-10 & Rekomendasi Klinis ──────────────────
        hasil_cdss_dokter = st.session_state.get("hasil_cdss_dokter")

        if hasil_cdss_dokter and hasil_cdss_dokter.get("status") == "success":
            ctx           = hasil_cdss_dokter.get("clinical_context", {})
            diagnosa_list = hasil_cdss_dokter.get("diagnosa_list", [])
            rekomendasi   = hasil_cdss_dokter.get("rekomendasi", [])
            numeric       = hasil_cdss_dokter.get("numeric_findings", {})

            # ── Alert Kritis ─────────────────────────────────────────────────
            for kr in [r for r in rekomendasi if r.get("prioritas") == "KRITIS"]:
                st.error(f"🚨 KRITIS [{kr.get('kategori','')}]: {kr['deskripsi']}")

            # ── Metric Cards Tanda Vital ──────────────────────────────────────
            if numeric:
                _LBL = {
                    "heart_rate":"HR","systolic_bp":"TD Sistolik",
                    "diastolic_bp":"TD Diastolik","spo2":"SpO2",
                    "respiratory_rate":"RR","temperature":"Suhu",
                    "gds":"GDS","laktat":"Laktat",
                    "creatinine":"Kreatinin","kalium":"K+",
                }
                _UNT = {
                    "heart_rate":"bpm","systolic_bp":"mmHg",
                    "diastolic_bp":"mmHg","spo2":"%",
                    "respiratory_rate":"x/mnt","temperature":"°C",
                    "gds":"mg/dL","laktat":"mmol/L",
                    "creatinine":"mg/dL","kalium":"mEq/L",
                }
                ttv_cols = st.columns(min(len(numeric), 6))
                for i, (k, v) in enumerate(list(numeric.items())[:6]):
                    ttv_cols[i].metric(_LBL.get(k, k.upper()), f"{v} {_UNT.get(k,'')}")

            # ── Panel Diagnosa per Ekspander ──────────────────────────────────
            st.markdown("---")
            st.markdown("#### 🔬 Diagnosa Kerja & Protokol PPK")

            _ICON_DX = {
                "KRITIS":"🔴","TINGGI":"🟠","SEGERA":"🟡",
                "SEDANG":"🟢","PERHATIAN":"⚠️",
            }
            _CAT_ORDER = [
                "Pemeriksaan Awal",
                "Reperfusi","Defibrilasi","Kardioversi","CPR","Airway",
                "Antiplatelet","Antikoagulan","Vasopressor","Inotropik",
                "Vasodilator","Diuretik","Ventilasi","Antiaritmia",
                "Rate Control","Rhythm Control","Antibiotik","Resusitasi",
                "Oksigen","Analgetik","Nitrat","Morfin","Cairan","Statin",
                "Beta-blocker","ACE-I/ARB","Steroid","Glukosa","Nutrisi",
                "Sedasi","Koreksi K+","Stabilisasi Membran","Eliminasi",
                "Koreksi Asidosis","Dialisis","RRT","Stop Nefrotoksin",
                "MCS","NMBA","ECMO","DVT Profilaksis","Source Control",
                "TD Control","Target TD","Trombektomi","Neuroproteksi","tPA",
                "Evaluasi Invasif","Anti-iskemia","Koreksi",
                "Kardioversif","Klonidin","Labetolol","Vasodilatasi IV",
                "Monitoring","Perhatian / KI","Umum",
            ]
            _PRIO_RANK = {"KRITIS":0,"SEGERA":1,"TINGGI":2,"SEDANG":3,"PERHATIAN":4}

            if diagnosa_list:
                for dx in diagnosa_list:
                    ppk_nama   = dx.get("nama_ppk", "")
                    target_wkt = dx.get("target_waktu", "")
                    recs_dx = [
                        r for r in rekomendasi
                        if (dx["kode"] in r.get("kode", ""))
                        or (ppk_nama and r.get("detail", "") == ppk_nama)
                    ]
                    with st.expander(
                        f"**`{dx['kode']}`** — {dx['nama']}   |   _{dx['kategori']}_",
                        expanded=True,
                    ):
                        c1, c2 = st.columns([2, 3])
                        if ppk_nama:
                            c1.caption(f"📋 **{ppk_nama}**")
                        if target_wkt:
                            c2.warning(f"⏱️ {target_wkt}")
                        if recs_dx:
                            recs_sorted = sorted(
                                recs_dx,
                                key=lambda r: (
                                    _CAT_ORDER.index(r.get("kategori","Umum"))
                                    if r.get("kategori","Umum") in _CAT_ORDER else 99,
                                    _PRIO_RANK.get(r.get("prioritas",""), 5),
                                ),
                            )
                            cur_cat = None
                            for r in recs_sorted:
                                kat  = r.get("kategori","Umum")
                                prio = r.get("prioritas","")
                                desc = r.get("deskripsi","")
                                if kat != cur_cat:
                                    st.markdown(f"**{kat}**")
                                    cur_cat = kat
                                st.write(f"{_ICON_DX.get(prio,'⚪')} [{prio}] {desc}")
                        else:
                            st.caption("Tidak ada detail protokol untuk kode ini.")
            else:
                st.info(
                    "Tidak ada diagnosa ICD-10 terdeteksi. "
                    "Coba tambahkan kata kunci klinis (misal: STEMI, sepsis, "
                    "gagal jantung) di kolom S/O."
                )

            # ── Alert Vital Tambahan ──────────────────────────────────────────
            alert_extra = [
                r for r in rekomendasi
                if not any(
                    dx["kode"] in r.get("kode","") or
                    (r.get("detail","") == dx.get("nama_ppk","") and dx.get("nama_ppk",""))
                    for dx in diagnosa_list
                )
            ]
            if alert_extra:
                st.markdown("---")
                st.markdown("#### ⚡ Alert Klinis Berbasis Tanda Vital")
                for r in alert_extra:
                    st.write(
                        f"{_ICON_DX.get(r.get('prioritas',''),'⚪')} "
                        f"[{r.get('prioritas','')}] "
                        f"**{r.get('kategori','')}** — {r.get('deskripsi','')}"
                    )

            # ── Metadata ─────────────────────────────────────────────────────
            ts       = ctx.get("timestamp_analisis","")
            ppk_used = ctx.get("ppk_digunakan",[])
            if ts or ppk_used:
                st.caption(
                    f"🕐 Analisis: {ts}  |  "
                    f"PPK: {', '.join(ppk_used) if ppk_used else '-'}"
                )

        else:
            with st.container(border=True):
                st.markdown("#### 🔬 Panel Diagnosis ICD-10 & CDSS Dokter")
                if not CDSS_DOKTER_AVAILABLE:
                    st.warning(
                        "⚠️ Modul `cdss_dokter.py` belum ditemukan. "
                        "Salin ke `modules/services/cdss_dokter.py`."
                    )
                else:
                    st.info(
                        "📝 Isi kolom **S** dan **O** di bawah, lalu tekan "
                        "**🔬 Analisis ICD-10 & Tatalaksana** untuk mendapatkan "
                        "diagnosa kerja, protokol PPK, dan rekomendasi CDSS."
                    )
    elif IS_PERAWAT:
        # Panel Perawat: grafik tren SLKI
        render_grafik_slki(st.session_state.episode_id)
    elif IS_APOTEKER:
        # Panel Apoteker: tinjauan farmasi klinis — TIDAK memakai CDSS apa pun,
        # murni dokumentasi manual (sesuai arahan: CDSS hanya utk Perawat & Dokter).
        with st.container(border=True):
            st.markdown("#### 💊 Panel Tinjauan Farmasi Klinis")
            st.caption(
                "Tinjau order obat dari dokter di panel **Order dari Dokter (CPOE)** di atas, "
                "lalu dokumentasikan tinjauan interaksi/dosis obat di kolom S/O & "
                "Catatan Farmasi di bawah."
            )
            if hasil_cdss_dokter_ctx := st.session_state.get("hasil_cdss_dokter"):
                icd_ctx = hasil_cdss_dokter_ctx.get("clinical_context", {}).get("icd10_terdeteksi", [])
                if icd_ctx:
                    st.info(
                        "ℹ️ Konteks diagnosis medis (ICD-10) dari dokter: " +
                        " • ".join(f"`{k}`" for k in icd_ctx)
                    )
    elif IS_GIZI:
        # Panel Ahli Gizi: asuhan gizi — murni dokumentasi manual, tanpa CDSS.
        with st.container(border=True):
            st.markdown("#### 🥗 Panel Asuhan Gizi")
            st.caption(
                "Catat asesmen status gizi pasien (BB/TB, riwayat diet, kondisi klinis terkait nutrisi) "
                "pada kolom S/O di bawah, lalu lengkapi Asesmen & Rencana Diet pada Catatan Asuhan Gizi."
            )
    else:
        with st.container(border=True):
            st.info(f"ℹ️ Tidak ada panel klinis khusus untuk role **{_current_role}** pada halaman ini.")
    ...
    def _load_s_text() -> None:
        st.session_state["_s_text_area"] = st.session_state.get("s_text_area", "")

    def _store_s_text() -> None:
        st.session_state["s_text_area"] = st.session_state["_s_text_area"]

    def _reset_s_text() -> None:
        st.session_state["s_text_area"] = ""
        st.session_state["_s_text_area"] = ""

    def _load_o_text() -> None:
        st.session_state["_o_text_area"] = st.session_state.get("o_text_area", "")

    def _store_o_text() -> None:
        st.session_state["o_text_area"] = st.session_state["_o_text_area"]

    def _reset_o_text() -> None:
        st.session_state["o_text_area"] = ""
        st.session_state["_o_text_area"] = ""

    def _do_translate_s() -> None:
        """Callback on_click tombol Translate S — berjalan SEBELUM render ulang."""
        raw = st.session_state.get("_s_text_area", "") or st.session_state.get("s_text_area", "")
        raw = raw.strip()
        if not raw:
            st.session_state["_translate_s_msg"] = ("warning", "⚠️ Kolom S kosong.")
            return
        try:
            result = translate_to_indonesian(raw, field="S")
    ...
    def _do_translate_o() -> None:
        """Callback on_click tombol Translate O — berjalan SEBELUM render ulang."""
        raw = st.session_state.get("_o_text_area", "") or st.session_state.get("o_text_area", "")
        raw = raw.strip()
        if not raw:
            st.session_state["_translate_o_msg"] = ("warning", "⚠️ Kolom O kosong.")
            return
        try:
            result = translate_to_indonesian(raw, field="O")
            if result and result != raw:
                st.session_state["o_text_area"] = result
    ...
    col1, col2 = st.columns(2)
    with col1:
        _load_s_text()  # salin nilai permanen -> key widget, SEBELUM widget dibuat
        s_input = st.text_area(
            ...
            height=150,
            key="_s_text_area",
            on_change=_store_s_text,
        )
        _s_btn_col1, _s_btn_col2 = st.columns([1, 1])
        with _s_btn_col1:
            st.button(
        ...
        with _s_btn_col2:
            ...
            if _msg_s:
                ...
                if kind == "success":
                    ...
                    st.error(txt)
    with col2:
        _load_o_text()  # salin nilai permanen -> key widget, SEBELUM widget dibuat
        o_input = st.text_area(
            ...
            height=150,
            key="_o_text_area",
            on_change=_store_o_text,
        )
        _o_btn_col1, _o_btn_col2 = st.columns([1, 1])
        with _o_btn_col1:
            st.button(
    ...
    # ── Role-based tombol analisis utama (FIX RBAC: 4 profesi inti) ─────────────
    if IS_DOKTER:
        # ── TOMBOL DOKTER: Analisis ICD-10 & Tatalaksana (CDSS — TIDAK DIUBAH) ──
        btn_col1, btn_col2 = st.columns([2, 1])
        with btn_col1:
            btn_dokter_analisis = st.button(
                "🔬 Analisis ICD-10 & Tatalaksana",
                type="primary",
                use_container_width=True,
                help="Analisis data S/O untuk deteksi kode ICD-10 dan rekomendasi tatalaksana medis.",
            )
        with btn_col2:
            btn_dokter_reset = st.button(
                "🗑️ Reset Analisis",
                use_container_width=True,
                help="Hapus hasil analisis ICD-10 terakhir.",
            )

        if btn_dokter_reset:
            st.session_state.pop("hasil_cdss_dokter", None)
            st.session_state.pop("diagnosa_aktif", None)
            st.rerun()

        if btn_dokter_analisis:
            if not s_input.strip() and not o_input.strip():
                st.warning("⚠️ Mohon isi minimal satu kolom data S atau O.")
            else:
                with st.spinner("🔬 Menganalisis ICD-10 dan menyusun rekomendasi tatalaksana..."):
                    if CDSS_DOKTER_AVAILABLE and _cdss_dokter_analyze:
                        hasil_cdss_dokter = _cdss_dokter_analyze(s_input, o_input)
                    else:
                        # Fallback ke engine lama
                        hasil_cdss_dokter = analyze_clinical_trends_improved(s_input, o_input)

                st.session_state["hasil_cdss_dokter"] = hasil_cdss_dokter

                # Sinkronisasi diagnosa_aktif — format baru cdss_dokter
                diagnosa_list_baru = hasil_cdss_dokter.get("diagnosa_list", [])
                icd_list = hasil_cdss_dokter.get("clinical_context", {}).get("icd10_terdeteksi", [])

                if diagnosa_list_baru:
                    st.session_state["diagnosa_aktif"] = [
                        {
                            "kode":          dx["kode"],
                            "kode_icd10":    dx["kode"],
                            "nama":          dx["nama"],
                            "nama_diagnosa": dx["nama"],
                        }
                        for dx in diagnosa_list_baru
                    ]
                elif icd_list:
                    st.session_state["diagnosa_aktif"] = [
                        {"kode": k, "kode_icd10": k, "nama": k, "nama_diagnosa": k}
                        for k in icd_list
                    ]

                # Reset narasi A&P agar di-generate ulang dari data segar
                st.session_state.pop("soap_A_dokter", None)
                st.session_state.pop("soap_P_dokter", None)
                st.session_state.pop("draft_cppt_dokter", None)
                # Reset ceklist ICD-10 agar diinisialisasi ulang dari hasil analisis baru
                st.session_state.pop("selected_icd10_dokter", None)
                st.session_state.pop("_icd10_fp_lama", None)
                st.session_state.pop("_regen_ap_dokter", None)
                # Increment widget version agar text_area render ulang dengan value baru
                st.session_state["_ap_widget_ver"] = (
                    st.session_state.get("_ap_widget_ver", 0) + 1
                )

                if hasil_cdss_dokter.get("status") == "success":
                    jumlah_dx   = len(diagnosa_list_baru or icd_list)
                    jumlah_recs = len(
                        hasil_cdss_dokter.get("rekomendasi",
                        hasil_cdss_dokter.get("recommendations", []))
                    )
                    st.success(
                        f"✅ Analisis selesai: **{jumlah_dx} diagnosa ICD-10** terdeteksi, "
                        f"**{jumlah_recs} rekomendasi** tatalaksana dihasilkan."
                    )
                else:
                    st.warning(
                        "⚠️ CDSS tidak mendeteksi kode ICD-10 spesifik. "
                        "Coba tambahkan kata kunci klinis yang lebih spesifik di S/O."
                    )
                st.rerun()

    elif IS_PERAWAT:
        # ── TOMBOL PERAWAT: Formulasikan Standar 3S (CDSS — TIDAK DIUBAH) ───────
        ...
        # ── TOMBOL APOTEKER: Susun Catatan Pelayanan Kefarmasian ────────────────
        # Bukan CDSS — dokumentasi manual murni (tinjauan resep/interaksi obat).
        if st.button("💾 Susun Catatan Pelayanan Kefarmasian", type="primary"):
            if not s_input.strip() and not o_input.strip():
                st.warning("⚠️ Mohon isi minimal satu kolom data S atau O.")
            else:
                st.session_state["draft_cppt_apoteker"] = generate_catatan_apoteker(s_input, o_input)
                st.success("✅ Draf catatan farmasi tersusun. Tinjau & lengkapi di bawah.")
                st.rerun()

    elif IS_GIZI:
        # ── TOMBOL AHLI GIZI: Susun Catatan Asuhan Gizi ─────────────────────────
        # Bukan CDSS — dokumentasi manual murni (asesmen & rencana diet).
        if st.button("💾 Susun Catatan Asuhan Gizi", type="primary"):
            if not s_input.strip() and not o_input.strip():
                st.warning("⚠️ Mohon isi minimal satu kolom data S atau O.")
            else:
                st.session_state["draft_cppt_gizi"] = generate_catatan_gizi(s_input, o_input)
                st.success("✅ Draf catatan asuhan gizi tersusun. Tinjau & lengkapi di bawah.")
                st.rerun()

    else:
        st.caption("ℹ️ Tidak ada aksi pencatatan khusus untuk role ini.")

    if st.session_state.daftar_asuhan and IS_PERAWAT:
    ...
    if st.session_state.draft_cppt and IS_PERAWAT:
    ...
    # ── Panel CPPT Dokter: Assessment Medis & Plan Tatalaksana ──────────────
    if IS_DOKTER:
        hasil_cdss_dokter = st.session_state.get("hasil_cdss_dokter")
        if hasil_cdss_dokter and hasil_cdss_dokter.get("status") == "success":
            st.write("---")
            st.subheader("✍️ Assessment Medis & Rencana Tatalaksana (A & P)")

            # ── Ambil data format baru cdss_dokter (dengan fallback format lama) ──
            diagnosa_list = hasil_cdss_dokter.get("diagnosa_list", [])
            rekomendasi   = hasil_cdss_dokter.get("rekomendasi",
                            hasil_cdss_dokter.get("recommendations", []))
            numeric       = hasil_cdss_dokter.get("numeric_findings", {})
            ctx           = hasil_cdss_dokter.get("clinical_context", {})
            icd_list      = ctx.get("icd10_terdeteksi", [])
            ...
            _PRIO2 = {"KRITIS":0,"SEGERA":1,"TINGGI":2,"SEDANG":3,"PERHATIAN":4}

            # ── ☑️ CEKLIST DIAGNOSA KERJA ICD-10 & PPK ───────────────────────
            # Dokter dapat memilih mana diagnosa/PPK yang akan masuk ke draft CPPT
            st.subheader("☑️ Diagnosa Kerja & Protokol PPK")
            st.caption(
                "CDSS mendeteksi diagnosa berikut. "
                "**Hilangkan centang** untuk mengecualikan diagnosa tertentu dari draft CPPT. "
                "Diagnosis yang tidak dicentang tetap ditampilkan sebagai referensi."
            )

            # Inisialisasi session_state untuk pilihan ICD-10 dokter
            if "selected_icd10_dokter" not in st.session_state:
                if diagnosa_list:
                    st.session_state["selected_icd10_dokter"] = {
                        dx["kode"] for dx in diagnosa_list
                    }
                elif icd_list:
                    st.session_state["selected_icd10_dokter"] = set(icd_list)
                else:
                    st.session_state["selected_icd10_dokter"] = set()

            # Tentukan daftar yang akan dirender sebagai baris ceklist
            _semua_dx_dokter = diagnosa_list if diagnosa_list else [
                {"kode": k, "nama": k, "nama_ppk": "", "kategori": "", "target_waktu": ""}
                for k in icd_list
            ]

            if _semua_dx_dokter:
                # Grup berdasarkan kategori agar lebih rapi
                _kategori_dx = {}
                for _dx in _semua_dx_dokter:
                    _kat = _dx.get("kategori", "Umum")
                    _kategori_dx.setdefault(_kat, []).append(_dx)

                for _kat_label, _dxs in _kategori_dx.items():
                    if len(_kategori_dx) > 1:
                        st.markdown(f"**📂 {_kat_label}**")
                    for _dx in _dxs:
                        _kode     = _dx.get("kode", "")
                        _nama     = _dx.get("nama", _kode)
                        _ppk      = _dx.get("nama_ppk", "")
                        _target   = _dx.get("target_waktu", "")

                        _col_chk, _col_info = st.columns([0.05, 0.95])
                        with _col_chk:
                            _is_sel = _kode in st.session_state["selected_icd10_dokter"]
                            _new_sel = st.checkbox(
                                label=f"Pilih {_kode}",
                                value=_is_sel,
                                key=f"chk_icd10_dok_{_kode}",
                                label_visibility="collapsed",
                            )
                            if _new_sel:
                                st.session_state["selected_icd10_dokter"].add(_kode)
                            else:
                                st.session_state["selected_icd10_dokter"].discard(_kode)

                        with _col_info:
                            _badge_color = "#28a745" if _kode in st.session_state["selected_icd10_dokter"] else "#6c757d"
                            _ppk_text    = f" &nbsp;|&nbsp; 📋 PPK: **{_ppk}**" if _ppk else ""
                            _target_text = f" &nbsp;·&nbsp; 🕐 *{_target}*" if _target else ""
                            st.markdown(
                                f"<span style='background:{_badge_color};color:#fff;"
                                f"padding:1px 7px;border-radius:4px;font-size:12px;"
                                f"font-weight:bold'>{_kode}</span> &nbsp; "
                                f"**{_nama}**{_ppk_text}{_target_text}",
                                unsafe_allow_html=True,
                            )

                # Ringkasan pilihan & tombol terapkan
                _dipilih_count   = len(st.session_state["selected_icd10_dokter"])
                _total_dx_count  = len(_semua_dx_dokter)
                st.caption(
                    f"✅ **{_dipilih_count} dari {_total_dx_count}** diagnosa dipilih untuk masuk ke draft CPPT."
                )

                _col_apply, _col_selall, _col_clearall = st.columns([2, 1, 1])
                with _col_apply:
                    if st.button(
                        "🔄 Terapkan Pilihan & Regenerasi A/P",
                        key="btn_apply_icd10_dokter",
                        type="primary",
                        use_container_width=True,
                        help="Regenerasi narasi Assessment (A) dan Plan (P) hanya dari diagnosa yang dicentang.",
                    ):
                        # Set flag regenerasi & hapus fingerprint agar A/P pasti di-rebuild
                        st.session_state["_regen_ap_dokter"] = True
                        st.session_state.pop("_icd10_fp_lama", None)
                        st.session_state.pop("draft_cppt_dokter", None)
                        st.rerun()
                with _col_selall:
                    if st.button("☑️ Pilih Semua", key="btn_selall_icd10_dok", use_container_width=True):
                        st.session_state["selected_icd10_dokter"] = {
                            _dx["kode"] for _dx in _semua_dx_dokter
                        }
                        st.session_state["_regen_ap_dokter"] = True
                        st.session_state.pop("_icd10_fp_lama", None)
                        st.session_state.pop("draft_cppt_dokter", None)
                        st.rerun()
                with _col_clearall:
                    if st.button("🔲 Hapus Semua", key="btn_clearall_icd10_dok", use_container_width=True):
                        st.session_state["selected_icd10_dokter"] = set()
                        st.session_state["_regen_ap_dokter"] = True
                        st.session_state.pop("_icd10_fp_lama", None)
                        st.session_state.pop("draft_cppt_dokter", None)
                ...
                if _dipilih_count == 0:
                    st.warning("⚠️ Pilih minimal satu diagnosa agar draft CPPT dapat dibuat.")

            else:
                st.info("ℹ️ Tidak ada diagnosa ICD-10 yang terdeteksi dari analisis CDSS.")

            st.write("---")

            # ── Terapkan filter pilihan ICD-10 ke diagnosa_list & icd_list ────
            # Filter ini berjalan SETIAP render, sehingga perubahan ceklist
            # langsung tercermin di narasi A/P tanpa perlu rerun tambahan.
            _selected_set = st.session_state.get("selected_icd10_dokter", set())
            _diagnosa_list_filtered = (
                [_dx for _dx in diagnosa_list if _dx.get("kode","") in _selected_set]
                if diagnosa_list else []
            )
            _icd_list_filtered = (
                [k for k in icd_list if k in _selected_set]
                if icd_list else []
            )

            # ── Fungsi helper generate A & P dari data terfilter ─────────────
            def _build_soap_A(dx_list, icd_lst, num, recs):
                lines = ["=== ASSESSMENT MEDIS ==="]
                if dx_list:
                    lines.append("Diagnosis Kerja:")
                    for dx in dx_list:
                        tag = f"  ({dx.get('nama_ppk','')})" if dx.get("nama_ppk") else ""
                        lines.append(f"  - [{dx['kode']}] {dx['nama']}{tag}")
                elif icd_lst:
                    lines.append(f"Kode ICD-10: {', '.join(icd_lst)}")
                if num:
                    ttv = ", ".join(
                        f"{_LBL2.get(k,k.upper())} {v} {_UNT2.get(k,'')}"
                        for k, v in num.items()
                    )
                    lines.append(f"Tanda Vital: {ttv}")
                kritis = [r for r in recs if r.get("prioritas", r.get("priority","")) == "KRITIS"]
                if kritis:
                    lines.append("Kondisi Kritis Terdeteksi:")
                    for kr in kritis:
                        lines.append(
                            f"  - [{kr.get('kategori','')}] "
                            f"{kr.get('deskripsi', kr.get('description',''))}"
                        )
                return "\n".join(lines)

            def _build_soap_P(dx_list, icd_lst, recs):
                lines = ["=== RENCANA TATALAKSANA ==="]
                if dx_list:
                    for dx in dx_list:
                        ppk_nama = dx.get("nama_ppk","")
                        recs_dx = [
                        ...
                        lines.append(f"\n[{dx['kode']} — {dx['nama']}]")
                        if ppk_nama:
                            lines.append(f"Protokol: {ppk_nama}")
                        if dx.get("target_waktu"):
                            lines.append(f"Target: {dx['target_waktu']}")
                        for r in sorted(recs_dx, key=lambda r: _PRIO2.get(r.get("prioritas",""), 5)):
                            prio = r.get("prioritas", r.get("priority",""))
                            kat  = r.get("kategori",  r.get("category", ""))
                            desc = r.get("deskripsi", r.get("description",""))
                            lines.append(f"  [{prio}] {kat}: {desc}")
                elif recs:
                    from itertools import groupby as _gby
                    recs_s = sorted(recs, key=lambda r: r.get("category", r.get("kategori","")))
                    for cat, grp in _gby(recs_s, key=lambda r: r.get("category", r.get("kategori","Umum"))):
                        lines.append(f"\n[{cat}]")
                        for r in list(grp)[:5]:
                            prio = r.get("priority", r.get("prioritas",""))
                            desc = r.get("description", r.get("deskripsi",""))
                            lines.append(f"  [{prio}] {desc}")
                return "\n".join(lines)

            # ── Tentukan apakah perlu regenerasi ─────────────────────────────
            # Regenerasi terjadi ketika:
            # 1) soap_A/P belum ada (generate pertama kali), ATAU
            # 2) flag _regen_ap di session_state di-set True oleh tombol "Terapkan"
            _regen_ap = st.session_state.pop("_regen_ap_dokter", False)

            # Simpan "sidik jari" pilihan terakhir untuk deteksi perubahan
            _fp_baru = frozenset(_selected_set)
            _fp_lama = st.session_state.get("_icd10_fp_lama", None)
            _pilihan_berubah = (_fp_baru != _fp_lama)

            if (not st.session_state.get("soap_A_dokter")) or _regen_ap or _pilihan_berubah:
                st.session_state["soap_A_dokter"] = _build_soap_A(
                    _diagnosa_list_filtered, _icd_list_filtered, numeric, rekomendasi
                )
                st.session_state["soap_P_dokter"] = _build_soap_P(
                    _diagnosa_list_filtered, _icd_list_filtered, rekomendasi
                )
                st.session_state["_icd10_fp_lama"] = _fp_baru
                # Naikkan counter agar key widget berubah → Streamlit wajib render ulang
                # dengan value= baru (bukan nilai lama dari internal widget state)
                st.session_state["_ap_widget_ver"] = (
                    st.session_state.get("_ap_widget_ver", 0) + 1
                )

            _ap_ver = st.session_state.get("_ap_widget_ver", 0)

            # ── Editor kolom A & P ────────────────────────────────────────────
            col_A_dok, col_P_dok = st.columns(2)
            with col_A_dok:
                soap_a_dokter = st.text_area(
                    "A (Assessment / Diagnosis Medis)",
                    value=st.session_state.get("soap_A_dokter", ""),
                    height=220,
                    key=f"_soap_a_dokter_widget_{_ap_ver}",
                )
                st.session_state["soap_A_dokter"] = soap_a_dokter
                st.session_state["soap_A"]         = soap_a_dokter

            with col_P_dok:
                soap_p_dokter = st.text_area(
                    "P (Plan / Tatalaksana Medis)",
                    value=st.session_state.get("soap_P_dokter", ""),
                    height=220,
                    key=f"_soap_p_dokter_widget_{_ap_ver}",
                )
                st.session_state["soap_P_dokter"] = soap_p_dokter
                st.session_state["soap_P"]         = soap_p_dokter

            st.write("---")
            col_save_dok, col_clear_dok = st.columns([2, 1])
            with col_save_dok:
                if st.button("✅ Simpan Assessment & Plan Dokter", type="primary"):
                    nama_dokter  = st.session_state.get("nama_lengkap", st.session_state.get("user_id","Dokter")).upper()
                    episode_id   = st.session_state.get("episode_id","-")
                    shift        = st.session_state.get("shift","-")
                    pasien_nama  = st.session_state.get("pasien_nama","-")
                    pasien_rm    = st.session_state.get("pasien_no_rm","-")
                    pasien_ruang = st.session_state.get("pasien_ruangan","-")
                    _recs_save   = hasil_cdss_dokter.get("rekomendasi",
                                   hasil_cdss_dokter.get("recommendations",[]))
                    _num_save    = hasil_cdss_dokter.get("numeric_findings",{})

                    draft  = "CATATAN PERKEMBANGAN PASIEN TERINTEGRASI (CPPT)\n"
                    draft += f"ID Episode : {episode_id} | Shift: {shift}\n"
                    draft += f"Tanggal    : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                    draft += f"Nama Pasien: {pasien_nama} | No. RM: {pasien_rm} | Ruangan: {pasien_ruang}\n"
                    draft += "=" * 70 + "\n\n"
                    ...
                    if _num_save:
                        ttvs = ", ".join(
                            f"{_LBL2.get(k,k)} {v} {_UNT2.get(k,'')}"
                            for k, v in _num_save.items()
                        )
                        draft += f"   Tanda Vital: {ttvs}\n"
                    draft += "\n"
                    draft += st.session_state.get("soap_A_dokter","").strip() + "\n\n"
                    draft += st.session_state.get("soap_P_dokter","").strip() + "\n\n"
                    kritis_save = [
                        r for r in _recs_save
                        if r.get("prioritas", r.get("priority","")) in ("KRITIS","TINGGI","SEGERA")
                    ]
                    if kritis_save:
                        draft += "TINDAKAN PRIORITAS:\n"
                        for r in kritis_save[:10]:
                            prio = r.get("prioritas", r.get("priority",""))
                            desc = r.get("deskripsi", r.get("description",""))
                            draft += f"  - [{prio}] {desc}\n"
                        draft += "\n"
                    draft += f"Divalidasi oleh: {nama_dokter} (DPJP)\n"

                    st.session_state["draft_cppt_dokter"] = draft
                    st.success(f"✅ Assessment & Plan tersimpan untuk Episode `{episode_id}`.")
                    st.rerun()

            with col_clear_dok:
                if st.button("🗑️ Reset Narasi A & P"):
                    st.session_state.pop("soap_A_dokter", None)
                    st.session_state.pop("soap_P_dokter", None)
                    st.session_state.pop("draft_cppt_dokter", None)
                    st.session_state.pop("_icd10_fp_lama", None)
    ...
    # ── Preview & Edit Draft CPPT Dokter ────────────────────────────────────
    if st.session_state.get("role", "Perawat") == "Dokter":
        draft_cppt_dokter = st.session_state.get("draft_cppt_dokter")
        if draft_cppt_dokter:
            st.write("---")
            st.subheader("📄 Preview & Edit Draft CPPT Dokter")
            edited_draft = st.text_area(
                "Narasi CPPT Final (dapat diedit sebelum disimpan):",
                value=draft_cppt_dokter,
                height=400,
                key="_draft_cppt_dokter_widget",
            )
            st.session_state["draft_cppt_dokter"] = edited_draft

            col_final1, col_final2 = st.columns([2, 1])
            with col_final1:
                if st.button("✅ Simpan Permanen CPPT Dokter", type="primary"):
                    st.success(
                        f"✅ CPPT Dokter tersimpan permanen untuk Episode "
                        f"`{st.session_state.get('episode_id', '-')}`."
                    )
            with col_final2:
                if st.button("🗑️ Hapus Draft CPPT"):
                    st.session_state.pop("draft_cppt_dokter", None)
                    st.rerun()

    # ── Preview & Edit Draft Catatan Apoteker (FIX RBAC — panel baru) ───────────
    if IS_APOTEKER:
        draft_cppt_apoteker = st.session_state.get("draft_cppt_apoteker")
        if draft_cppt_apoteker:
            st.write("---")
            st.subheader("📄 Preview & Edit Catatan Pelayanan Kefarmasian")
            edited_apoteker = st.text_area(
                "Narasi Catatan Farmasi (dapat diedit sebelum disimpan):",
                value=draft_cppt_apoteker,
                height=350,
                key="_draft_cppt_apoteker_widget",
            )
            st.session_state["draft_cppt_apoteker"] = edited_apoteker

            col_apt1, col_apt2 = st.columns([2, 1])
            with col_apt1:
                if st.button("✅ Simpan Permanen Catatan Farmasi", type="primary"):
                    st.success(
                        f"✅ Catatan farmasi tersimpan permanen untuk Episode "
                        f"`{st.session_state.get('episode_id', '-')}`."
                    )
            with col_apt2:
                if st.button("🗑️ Hapus Draft Farmasi"):
                    st.session_state.pop("draft_cppt_apoteker", None)
                    st.rerun()

    # ── Preview & Edit Draft Catatan Asuhan Gizi (FIX RBAC — panel baru) ────────
    if IS_GIZI:
        draft_cppt_gizi = st.session_state.get("draft_cppt_gizi")
        if draft_cppt_gizi:
            st.write("---")
            st.subheader("📄 Preview & Edit Catatan Asuhan Gizi")
            edited_gizi = st.text_area(
                "Narasi Catatan Asuhan Gizi (dapat diedit sebelum disimpan):",
                value=draft_cppt_gizi,
                height=350,
                key="_draft_cppt_gizi_widget",
            )
            st.session_state["draft_cppt_gizi"] = edited_gizi

            col_giz1, col_giz2 = st.columns([2, 1])
            with col_giz1:
                if st.button("✅ Simpan Permanen Catatan Asuhan Gizi", type="primary"):
                    st.success(
                        f"✅ Catatan asuhan gizi tersimpan permanen untuk Episode "
                        f"`{st.session_state.get('episode_id', '-')}`."
                    )
            with col_giz2:
                if st.button("🗑️ Hapus Draft Gizi"):
                    st.session_state.pop("draft_cppt_gizi", None)
                    st.rerun()
...
# ENTRY POINT
# =============================================================================
# Login sudah ditangani di blok atas (render_login_form + st.stop()).
# Jika eksekusi sampai sini, user sudah terautentikasi.
